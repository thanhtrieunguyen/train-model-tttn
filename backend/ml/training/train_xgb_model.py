...
class XGBoostTrainer:
    def __init__(self):
        self.model = xgb.XGBRegressor(
            n_estimators=300,          # Tăng số lượng cây để tối ưu hóa
            learning_rate=0.05,        # Học chậm hơn để đạt kết quả tốt hơn
            max_depth=8,               # Kiểm soát độ sâu để tránh overfitting
            subsample=0.8,             # Lấy 80% dữ liệu mỗi vòng để tránh overfitting
            colsample_bytree=0.8,      # Lấy 80% số đặc trưng cho mỗi cây
            reg_alpha=0.1,             # Regularization L1
            reg_lambda=1.0,            # Regularization L2
            gamma=0.1,                 # Phạt chia nhánh không hiệu quả
            random_state=42,
            n_jobs=-1
        )
    ...
    def plot_wind_direction_predictions(self, y_test_sin, y_test_cos, y_pred_sin, y_pred_cos, target_name):
        ...
        plt.title(f'XGBoost: Dự báo hướng gió: 100 mẫu đầu tiên\nMAE: {mae:.2f}°')
        plt.legend()
    ...
    def plot_predictions(self, y_test, y_pred, target_name):
        ...
        plt.title(f'XGBoost: Dự báo {target_name}: 100 mẫu đầu tiên')
...
def train_xgb_model():
    ...
    print("Quá trình huấn luyện mô hình XGBoost đã hoàn tất!")
